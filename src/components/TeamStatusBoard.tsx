...
const TeamStatusBoard: React.FC<TeamStatusBoardProps> = ({ 
  ...
  const fetchTeamMembers = async () => {
    ...
    try {
      // Get all company members with their profiles
      ...
      // Combine member data with their latest status
      const teamData: TeamMember[] = members?.map((member: any) => {
        // Find the latest status for this user
        const userStatuses = statuses?.filter(s => s.user_id === member.profiles?.id) || []
        const latestStatus = userStatuses.length > 0 ? userStatuses[0] : null
