  ...
  const handleCreateConversation = async () => {
    ...
    try {
      ...
      const participants = [
        ...
        ...selectedUsers.map(u => ({ conversation_id: conversation.id, user_id: u.id }))
      ]
      ...
      // Send initial system message for group conversations
      if (conversationType === 'group') {
        const participantNames = selectedUsers.map(u => u.full_name).join(', ')
        const systemMessage = {
          conversation_id: conversation.id,
          content: `${user?.user_metadata?.full_name || 'Someone'} created this group with ${participantNames}`,
          user_id: user?.id,
          is_system: true
        }
