...
async function presenceRoutes(fastify) {
    ...
    fastify.get('/:companyId', {
        ...
        try {
            ...
            const presence = await prisma_1.prisma.userPresence.findUnique({
                where: {
                    userId_companyId: {
                        userId,
                        companyId,
            ...
            if (!presence) {
                ...
                const defaultPresence = await prisma_1.prisma.userPresence.create({
                    data: {
                        userId,
                        companyId,
                        status: 'OFFLINE',
    ...
    fastify.put('/:companyId', {
        ...
        try {
            ...
            const presence = await prisma_1.prisma.userPresence.upsert({
                where: {
                    userId_companyId: {
                        userId,
                        companyId,
    ...
    fastify.get('/:companyId/members', {
        ...
        try {
            ...
            const presences = await prisma_1.prisma.userPresence.findMany({
                where: { companyId },
                include: {
                    user: {
                        select: {
